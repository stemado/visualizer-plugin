...
1. **`save(sessionId, html, title)`** — Called by session-manager on every `push_screen`. Writes the HTML file to disk as `NNN-timestamp-title.html` and appends an entry to `manifest.json`. Writes use temp-file-then-rename for crash safety. On Windows, retries on `EPERM`/`EACCES` (up to 3 attempts with 50ms delay) since antivirus or indexing services may briefly lock files.
...
3. **`generateGallery(sessionId)`** — Builds a self-contained `index.html` from the manifest and archived HTML files. This is a static page that works without any server. Failures are non-fatal — logged as warnings, never thrown. Can always be retried via the `generate_gallery` MCP tool.
...
- CSS-scaled iframe thumbnail preview
- Title
- Timestamp
...
1. Capture `process.cwd()` at startup and pass it as `projectDir` to `createMcpTools(manager, projectDir)`
2. Add `title: z.string().optional().describe('Screen title for archive labeling')` to the `push_screen` tool schema
3. Register new `generate_gallery` tool with schema: `{ session_id: z.string().describe('Session to generate gallery for') }`, description: `"Generate a static HTML gallery page for an archived session"`, returns `{ path: string }`

### `mcp-server.js`

Note: The existing exported function is `createMcpTools(manager)`, not `createMcpServer`.

1. `createMcpTools(manager)` → `createMcpTools(manager, projectDir)`. Constructs `const archive = new ArchiveManager(projectDir)` once, then passes it to both `new SessionManager({ ...opts, archive })` and every `createHttpServer(manager, sessionId, archive)` call.
...
4. `close_session` handler: call `archive.generateGallery(session_id)` **before** calling `manager.destroy(session_id)`, so session metadata is still available. Gallery generation failure is logged but does not prevent session cleanup.
5. `onTimeout` handler: call `archive.generateGallery(session_id)` before cleanup, same as `close_session`. Timeout-closed sessions should also get a gallery since the user may want to review them later.

### `session-manager.js`

1. Constructor: extend existing `opts` parameter to include `archive`: `new SessionManager({ timeoutMs, maxSessions, onTimeout, archive })`. Stores as `this.archive`.
2. `pushScreen(sessionId, html)` → `pushScreen(sessionId, html, title)`. After setting `currentHtml` in memory (unchanged behavior), calls `this.archive.save(sessionId, html, title)`. Archive save failure is logged but does not throw.
3. `destroy(id)` — add `closedReason` parameter: `destroy(id, reason = 'explicit')`. Calls `this.archive.closeSession(sessionId, reason, session.screenIndex)` to write `closed` timestamp, `closedReason`, and final `screenCount` to `session-meta.json` before deleting the session from the map. The `screenCount` value comes from `session.screenIndex` (the existing counter that increments on each push). The internal timeout in `_touch()` passes `reason = 'timeout'` when calling `destroy()`.
...
- **Full-document screens in gallery**: Gallery uses `<iframe srcdoc>` which correctly handles both fragments (wrapped in frame template) and full HTML documents.
